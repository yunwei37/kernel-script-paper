#!/usr/bin/env python3
"""Run traffic-driven XDP benchmarks against matched C/eBPF baselines."""
...
def check_prerequisites() -> str | None:
    ...
    for cmd in ["bpftool", "clang", "ip", "iperf3", "make"]:
...
def setup_namespace(idx: int) -> tuple[str, str, str, str, str, int]:
    pid = os.getpid() % 10000
    ns = f"kstraffic{pid}_{idx}"
    host_dev = f"kt{pid:04d}{idx:02d}h"
    peer_dev = f"kt{pid:04d}{idx:02d}n"
    host_ip = f"10.252.{idx}.1"
    peer_ip = f"10.252.{idx}.2"
    port = 31000 + ((os.getpid() + idx) % 20000)
    ...
    # Populate ARP before attaching XDP so the count map mostly sees benchmark traffic.
...
def trial(name: str, obj: Path, idx: int, needs_count: bool) -> dict[str, object]:
    ...
    try:
        ...
        if map_id is not None:
            ...
            result["xdp_map_count"] = count
            result["xdp_map_mpps"] = count / result["seconds"] / 1_000_000.0
        else:
            result["xdp_map_count"] = ""
            result["xdp_map_mpps"] = ""
        result["prog_id"] = prog_id
        result["map_id"] = map_id or ""
        result["oracle_passed"] = result["receiver_bytes"] > 0 and (
            not needs_count or int(result["xdp_map_count"]) > 0
        )
        return result
    finally:
...
def summarize(name: str, bench: str, implementation: str, obj: Path, needs_count: bool, offset: int) -> dict[str, object]:
    ...
    map_mpps = [float(row["xdp_map_mpps"]) for row in samples if row["xdp_map_mpps"] != ""]
    return {
        "name": name,
        "bench": bench,
        ...
        "xdp_map_count_samples": [row["xdp_map_count"] for row in samples],
        "xdp_map_mpps_samples": map_mpps,
        ...
        "median_xdp_map_mpps": median(map_mpps),
