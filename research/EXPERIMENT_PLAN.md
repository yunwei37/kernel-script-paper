...
Source/command: top-systems experiment-design follow-up after scheduler-extension verifier diagnostic
...
| C3 | Generated eBPF objects remain compatible with the local kernel verifier, can attach for an XDP subset, and perf_event/ringbuf/struct_ops examples can complete local lifecycle, object-level, workload, callback-reachability, generated-build, or boundary-diagnostic checks. | Local kernel/toolchain, generated repository examples, verifier-clean single-section XDP subset, perf_event examples, one ringbuf XDP object workload, one tcp-congestion struct_ops object with clean and loss-injected loopback TCP callback-flag workloads, two generated struct_ops userspace build repairs, and one scheduler-extension verifier diagnostic with no scheduler attach. | Strict verifier matrix, isolated XDP attach matrix, perf_event generated-loader and counter workloads, ringbuf submitted/received/drop oracle, struct_ops load/attach/detach, TCP socket workload, clean and loss-injected callback flags, skeleton build-repair oracle, and scheduler-extension generated-vs-five-callback-control verifier diagnostic. | partial |
| C4 | The observed XDP count runtime gap is caused by a specific map-update lowering choice, and generated objects can run matched local workloads for XDP, TC, perf_event counters, ringbuf emission, and tcp-congestion struct_ops checks, with scheduler-extension currently recorded as a negative verifier boundary. | XDP array-map count mechanism; local XDP/TC pass/count traffic including one longer stress rerun, perf_event page-fault counter, XDP ringbuf emission, tcp-congestion struct_ops load/attach/detach, loopback TCP transfer, clean callback-flag reachability for cong_avoid/cwnd_event, loss-injected reachability for ssthresh/cong_avoid/set_state/cwnd_event, local struct_ops generated-build repair, and scheduler-extension verifier diagnostic only. | Hand-written C/eBPF baseline, compiler-source patch ablation, map-count correctness oracle, traffic and stress results, perf_event counter result, ringbuf event-rate/loss result, struct_ops direct compatibility, TCP workload, callback-flag workload, skeleton repair results, and scheduler-extension loadall diagnostic. | partial |
...
- Workloads: repository examples, static invalid programs, XDP pass/count
  ...
  struct_ops skeleton build repair, and scheduler-extension verifier loadall
  diagnostic with no scheduler attach.
...
| B15 | C3/C4 | Scheduler-extension struct_ops verifier diagnostic | KS vs C/eBPF | verifier-load status, pinned programs, sched_ext state | C/eBPF loadall succeeds; generated outcome and failure class recorded; no scheduler attach | Runtime paragraph | done |
...
| R013 | supplement | Scheduler-extension struct_ops verifier diagnostic | `./experiments/run_sched_ext_verifier.py` | one generated object plus one five-callback C/eBPF control baseline, no attach | C/eBPF control object verifier-loads and generated failure class is recorded without changing sched_ext state | low | done |
...
- Workload generation: iperf3 TCP over isolated veth/netns for XDP and TC
  ...
  struct_ops examples; verifier loadall only for the scheduler-extension
  diagnostic, with no scheduler attach.
...
- The XDP and TC traffic experiments are local-host evidence, not NIC-rate
  benchmarks.
- The generated scheduler-extension object is not verifier-clean on this host;
  fixing that lowering gap and running a scheduler workload remain necessary
  before claiming scheduler-extension support. Every callback-level TCP
