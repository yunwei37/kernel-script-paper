...
|-------|------------------|---------|-------------------|
| C1 | `results/evaluation_summary.json`, `results/examples_summary.csv`, `results/source_footprint_summary.json`, `results/external_corpus_summary.json`, `results/external_corpus_audit.csv`, `results/external_port_summary.json` | partial | Centralizes generated project structure for repository examples, reports a local matched source-footprint proxy, adds source-only external feature context, and adds a small manual external XDP port/build/runtime portfolio: unique KernelScript application sources total 203 SLOC, C/eBPF object sources total 254 SLOC, C/libbpf baseline sources total 1105 SLOC when runner or loader files are included, the external scan covers 166 selected C/header files across 3 pinned public eBPF repositories with 14 tracked feature families observed and a 7/7 manual classifier spot-check, and all 6 external port variants pass 5 one-second traffic trials against original external C/eBPF objects with the map-counter pair incrementing XDP_PASS. |
