...
- Workloads: repository examples, static invalid programs, XDP pass/count
  ...
  lifecycle smoke, perf_event page-fault counter workload.
...
| B9 | C3/C4 | Ringbuf and struct_ops throughput workloads | KS vs C/libbpf | event rate, loss | workload-specific checker | Future table | planned |
...
| R007 | supplement | Ringbuf/struct_ops throughput baseline | to be implemented | 10 trials | matched C/libbpf exists | medium | planned |
...
- Workload generation: iperf3 TCP over isolated veth/netns for XDP and TC
  ...
  page-fault workload for perf_event counter runs.
...
- The XDP and TC traffic experiments are local-host evidence, not NIC-rate
  benchmarks.
- Ringbuf, struct_ops, broader perf_event workloads, and generated-loader
  throughput remain necessary before claiming general runtime equivalence.
...
| C3 | `results/verifier_matrix_summary.json`, `results/attach_matrix_summary.json`, `results/perf_event_loader_summary.json`, `results/perf_event_counter_summary.json` | partial | Most generated build-success objects load; verifier-clean single-section XDP objects attach; one generated perf_event loader completes attach/read/detach; perf_event counter objects run a page-fault workload. |
| C4 | `results/compiler_patch_ablation_summary.json`, `results/xdp_traffic_summary.json`, `results/tc_traffic_summary.json`, `results/perf_event_counter_summary.json` | partial | XDP count gap is tied to map-update lowering; local traffic covers XDP and TC pass/count plus a perf_event counter workload, but broader runtime equivalence remains unproven. |
