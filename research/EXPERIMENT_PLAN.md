...
- Workloads: repository examples, static invalid programs, XDP pass/count
  ...
  diagnostic, and an opt-in bounded scheduler-extension attach workload.
...
| B16 | C3/C4 | Scheduler-extension attach workload | KS vs C/eBPF | register status, sched_ext state, bounded workload, unregister status | both toy schedulers keep sched_ext enabled during workload and clean up to disabled | Runtime paragraph | done |
...
| R014 | supplement | Scheduler-extension attach workload | `./experiments/run_sched_ext_attach.py --allow-host-scheduler` | C/eBPF and generated toy FIFO schedulers, 0.75s CPU workload | both variants register, stay enabled through workload, unregister, and return sched_ext to disabled | medium | done |
...
- Workload generation: iperf3 TCP over isolated veth/netns for XDP and TC
  ...
  and opt-in register/workload/unregister for one toy scheduler-extension
  policy.
...
- The generated scheduler-extension object is verifier-clean and one toy
  scheduler workload attaches on this host, but scheduler-policy quality,
