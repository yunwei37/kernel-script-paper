...
Source/command: top-systems experiment-design follow-up after external port check

# Experiment Plan: KernelScript Paper

## Thesis

KernelScript is useful as a prototype artifact for studying whether a typed
unified-source model can centralize multi-artifact eBPF project structure and
selected pre-load checks while retaining ordinary libbpf-compatible artifacts
with measurable local compatibility and runtime behavior.

## Paper Type

- Type: systems prototype and artifact evaluation.
- Target venue: top systems conference audience.
- Artifact status: prototype plus reproducible paper harness.
...
|----|-------|-------|------------------------|--------|
| C1 | Unified source centralizes eBPF project structure. | Repository examples, generated artifacts, matched local workload source files, pinned external eBPF source trees for source-only feature context, and a small manual external XDP portfolio. | Build matrix, feature coverage, generated SLOC expansion, matched source-footprint proxy, external source-corpus feature scan, and external port/build/runtime check. | partial |
...
- Workloads: repository examples, matched source-footprint rows, external
  source-corpus file scan, a pinned external XDP tutorial portfolio, static invalid programs, XDP pass/count
  ...
  load/attach/detach compatibility, loopback TCP workload, clean and
  loss-injected callback flags for tcp-congestion struct_ops, version-aware
  struct_ops skeleton build repair, scheduler-extension verifier loadall
  diagnostic, and an opt-in bounded scheduler-extension attach/progress workload.
...
| B14 | C3/C4 | Callback-flag tcp-congestion struct_ops workload | KS vs C/libbpf | TCP algorithm selection, bytes transferred, required callback flags | clean profile sets cong_avoid/cwnd_event; loss profile sets ssthresh/cong_avoid/set_state/cwnd_event | Runtime paragraph | done |
...
| B18 | C1 | Change-amplification micro-edits | checked-in KS and hand-written C/libbpf before/after fixtures | changed files, edit sites, changed LOC, required kernel/userspace/skeleton synchronization | fixture diffs reconcile to CSV/JSON | Generated-structure paragraph | done |
| B19 | C1 | External source-corpus feature context | pinned public eBPF source trees | selected-file count, SLOC, roles, `SEC()` sections, feature markers, manual classifier spot-check | pinned commits fetch; source classifier output reconciles to CSV/JSON; audit samples match expected markers | Generated-structure paragraph | done |
| B20 | C1 | External XDP portfolio port/build/runtime | manual KS ports vs original external C/eBPF sources | KS generated-Makefile builds, original C/eBPF direct clang compiles, XDP attach, iperf3 receiver Gb/s, XDP_PASS map-key Mpps for map-counter workload | all 6 objects pass traffic and the map-counter pair increments the same map key; semantic runtime sanity only, not C4 performance evidence | External port paragraph | done |
...
| R012 | supplement | Callback-flag tcp-congestion struct_ops workload | `./experiments/run_struct_ops_callback_workload.py` | 10 clean 4MiB trials plus 5 loss-injected 4MiB trials per variant | generated and C/eBPF objects transfer all bytes; clean profile sets cong_avoid/cwnd_event; loss profile sets ssthresh/cong_avoid/set_state/cwnd_event | low | done |
...
| R016 | supplement | Change-amplification micro-edits | `./experiments/run_change_amplification.py` | 4 checked-in matched micro-edits | diff metrics reconcile and manual sync-surface labels match fixture intent | low | done |
| R017 | supplement | External source-corpus feature context | `./experiments/run_external_corpus.py` | 3 pinned public repositories plus 7 manual audit samples | selected C/header source rows reconcile; source-only scope preserved; classifier spot-check has zero false-positive or false-negative feature labels | low | done |
| R018 | supplement | External XDP portfolio port/build/runtime | `./experiments/run_external_port.py` | 3 workloads, 6 variants, 5 x 1s iperf3 trials per variant | KernelScript ports build through generated Makefiles; original external C/eBPF sources compile directly with clang; all objects attach and pass traffic; map-counter pair increments XDP_PASS map key | low | done |
...
- Source-footprint policy: count maintained C/eBPF object sources and C/libbpf
  runner or loader files used by matched local baselines, but exclude generated
  headers, generated C, generated Makefiles, KernelScript library headers, and
...
- Workload generation: source-footprint row construction for local matched
  ...
  pinned public repository commits; a pinned external XDP tutorial portfolio;
  iperf3 TCP over isolated veth/netns for XDP and TC
  ...
  struct_ops examples; verifier loadall for the scheduler-extension diagnostic;
  and opt-in register/progress/unregister for one toy scheduler-extension
  policy.
...
- The plan adds source-only context from pinned public eBPF repositories and one
  manual external XDP port/build/runtime portfolio, but it still does
  not automate translation, cover broad production eBPF applications, or measure
  developer effort.
...
- The generated scheduler-extension object is verifier-clean and one toy
  scheduler progress/fairness workload attaches on this host, but scheduler-policy quality,
  performance, and portability remain untested. Every callback-level TCP
  behavior beyond the clean and loss-injected profiles, broader libbpf-version
  coverage for skeleton generation, broader perf_event workloads, broader
  generated-dispatch-loop throughput, and larger or non-local stress runs also
...
|-------|------------------|---------|-------------------|
| C1 | `results/evaluation_summary.json`, `results/examples_summary.csv`, `results/source_footprint_summary.json`, `results/change_amplification_summary.json`, `results/external_corpus_summary.json`, `results/external_corpus_audit.csv`, `results/external_port_summary.json` | partial | Centralizes generated project structure for repository examples, reports a local matched source-footprint proxy, adds a checked-in change-amplification study, adds source-only external feature context, and adds a small manual external XDP port/build/runtime portfolio: unique KernelScript application sources total 203 SLOC, C/eBPF object sources total 254 SLOC, C/libbpf baseline sources total 1105 SLOC when runner or loader files are included; across 5 matched micro-edits the medians are 1 changed file, 2 edit sites, and 6 changed lines for KernelScript versus 2, 7, and 30 for hand-written C/libbpf, with manual userspace synchronization required in 5/5 C/libbpf edits and 0/5 KernelScript edits; the external scan covers 166 selected C/header files across 3 pinned public eBPF repositories with 14 tracked feature families observed and a 7/7 manual classifier spot-check, and all 6 external port variants pass 5 one-second traffic trials against original external C/eBPF objects with the map-counter pair incrementing XDP_PASS. |
