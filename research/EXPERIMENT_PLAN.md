...
|----|-------|-------|------------------------|--------|
| C1 | Unified source centralizes eBPF project structure. | Repository examples, generated artifacts, matched local workload source files, pinned external eBPF source trees for source-only feature context, and one manual external XDP map-counter port. | Build matrix, feature coverage, generated SLOC expansion, matched source-footprint proxy, external source-corpus feature scan, and external port/build/runtime check. | partial |
...
- Workloads: repository examples, matched source-footprint rows, external
  source-corpus file scan, one external XDP map-counter port, static invalid programs, XDP pass/count
...
| B19 | C1 | External XDP map-counter port/build/runtime | manual KS port vs original external C/eBPF source | KS generated-Makefile build, original C/eBPF direct clang compile, XDP attach, iperf3 receiver Gb/s, XDP_PASS map-key Mpps | both objects pass traffic and increment the same map key; semantic runtime sanity only, not C4 performance evidence | External port paragraph | done |
...
| R017 | supplement | External XDP map-counter port/build/runtime | `./experiments/run_external_port.py` | 5 x 1s iperf3 trials per variant | KernelScript port builds through generated Makefile; original external C/eBPF source compiles directly with clang; both objects attach, pass traffic, and increment XDP_PASS map key | low | done |
...
- Workload generation: source-footprint row construction for local matched
  ...
  pinned public repository commits; one pinned external XDP map-counter port;
...
- The plan adds source-only context from pinned public eBPF repositories and one
  manual external XDP map-counter port/build/runtime check, but it still does
...
|-------|------------------|---------|-------------------|
| C1 | `results/evaluation_summary.json`, `results/examples_summary.csv`, `results/source_footprint_summary.json`, `results/external_corpus_summary.json`, `results/external_corpus_audit.csv`, `results/external_port_summary.json` | partial | Centralizes generated project structure for repository examples, reports a local matched source-footprint proxy, adds source-only external feature context, and adds one manual external XDP map-counter port/build/runtime check: unique KernelScript application sources total 203 SLOC, C/eBPF object sources total 254 SLOC, C/libbpf baseline sources total 1105 SLOC when runner or loader files are included, the external scan covers 166 selected C/header files across 3 pinned public eBPF repositories with 14 tracked feature families observed and a 7/7 manual classifier spot-check, and the external port passes 5 one-second traffic trials against the original external C/eBPF object. |
