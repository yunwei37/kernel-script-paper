...
- Workloads: repository examples, static invalid programs, XDP pass/count
  ...
  loss-injected callback flags for tcp-congestion struct_ops, and version-aware
  struct_ops skeleton build repair.
...
- The XDP and TC traffic experiments are local-host evidence, not NIC-rate
  benchmarks.
- Scheduler-extension struct_ops, every callback-level TCP behavior beyond the
  clean and loss-injected profiles, broader libbpf-version coverage for skeleton
