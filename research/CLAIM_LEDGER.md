...
|----|-------|-------|------------------------|--------|
| C1 | KernelScript centralizes recurring multi-artifact eBPF project structure while preserving conventional libbpf-compatible outputs. | Repository examples and matched local workload sources at commit `3b19cd2` on the recorded host toolchain. | Example compile/build matrix, generated SLOC expansion, matched source-footprint proxy, feature marker coverage, generated object inventory. | partial |
...
- Does the scheduler-extension toy workload result hold for richer scheduler
  ...
  C/libbpf baselines?
- Does the source-footprint proxy hold on external applications, and does a
  controlled developer study show lower implementation, debugging, or review
