...
| C3 | Generated eBPF objects remain compatible with the local kernel verifier, can attach for an XDP subset, and perf_event/ringbuf examples can complete local lifecycle or object-level workload checks. | Local Linux `6.15.11-061511-generic`, generated repository examples, verifier-clean single-section XDP subset, perf_event examples, and one ringbuf XDP object workload. | Strict `bpftool prog loadall` matrix with pinned program check, isolated netns/veth XDP attach matrix, perf_event generated-loader lifecycle smoke, perf_event page-fault counter workload, ringbuf submitted/received/drop oracle. | partial |
| C4 | The observed XDP count runtime gap is caused by a specific map-update lowering choice rather than the unified-source model alone, and generated objects can run matched local workloads for XDP, TC, perf_event counters, and ringbuf emission. | XDP array-map count mechanism; local XDP/TC pass/count traffic, perf_event page-fault counter, and XDP ringbuf emission only. | Hand-written C/eBPF baseline, generated current object, compiler-source patch ablation, map-count correctness oracle, microbench, XDP/TC traffic-driven results, perf_event counter result, ringbuf event-rate/loss result. | partial |

## Open Questions

- Does the result hold for struct_ops and broader perf_event workloads with matched C/libbpf baselines?
