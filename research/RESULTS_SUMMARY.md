...
Source/command: `./experiments/run_xdp_traffic.py` plus checked-in result summaries

# Results Summary

## Headline Result

The artifact now has a local traffic-driven XDP check in addition to
BPF_PROG_TEST_RUN microbenchmarks. On fresh veth/netns pairs with iperf3 TCP,
KernelScript and hand-written C/eBPF pass/count objects all pass the traffic
oracle. The count median is 17.4Gb/s for KernelScript and 17.5Gb/s for C/eBPF,
a 0.6% local throughput gap, with positive count-map rates of 1.51 and
1.52 Mpps respectively.
...
- The XDP traffic pass result includes one low KernelScript outlier
  ...
  comparison claim.
- The XDP traffic experiment does not exercise generated loaders, TC, perf
  events, ring buffers, or struct_ops.
