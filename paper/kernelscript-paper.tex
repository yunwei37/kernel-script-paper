...
Of 44 example programs, 43 compile successfully and 41 produce working libbpf projects.
% 在 44 个示例程序中，43 个编译成功，41 个生成可工作的 libbpf 项目。
...
The examples exercise the major eBPF features: XDP, TC, kprobe, tracepoint, perf\_event, maps, ring buffers, tail calls, kfuncs, and struct\_ops.
...
Of 41 fully built objects, 37 pass the verifier. The four failures are a reference-ownership leak, a map-creation failure, a missing kfunc symbol, and an object with no program section.
% 在 41 个完全构建的对象中，37 个通过验证器。4 个失败是引用所有权泄漏、映射创建失败、缺少 kfunc 符号和没有程序节的对象。
For XDP, all 27 eligible objects attach and detach successfully. Additional workloads exercise TC and struct\_ops.
% 对于 XDP，所有 27 个符合条件的对象都成功附加和分离。额外的工作负载执行 TC 和 struct\_ops。
