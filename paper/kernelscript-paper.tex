...
The comparative question is whether \tool{} catches defects the stock path still accepts. We replay three published, verifier-accepted exemplars from Heimdall~\cite{heimdall} (Listings~5--6): an XDP program typed with \texttt{struct \_\_sk\_buff}, a 16-byte update into an 8-byte map value, and a map lookup reinterpreted as an unrelated \texttt{u64}. For each defect we pair a buggy and corrected program in both \tool{} and C/libbpf, require an exact \tool{} diagnostic on the buggy source, and require the C object to clang-build, verifier-load, and pass a defect-specific \code{BPF_PROG_TEST_RUN} oracle over ten trials (map truncation to \texttt{\{1,2\}}, native-endian reinterpretation of \texttt{conn\{1,2\}} as \texttt{u64}, and successful field stores for the context pair).
% 对比问题是 \tool{} 是否捕获原有路径仍接受的缺陷。我们重放 Heimdall 的三个已发表、验证器接受的范例。
