...
All experiments run on Linux \KSKernelVersion{} with clang 18, bpftool 7.7, and libbpf 1.3.0. SLOC counts exclude blank lines, comments, and generated headers.
...
The compiler test suite contains \KSTotalUnitSuites{} suites and \KSTotalUnitTests{} tests with no failures. These tests cover all major subsystems from lexer to code generation.
% 编译器测试套件包含 \KSTotalUnitSuites{} 个套件和 \KSTotalUnitTests{} 个测试，无失败。这些测试覆盖从词法分析器到代码生成的所有主要子系统。
...
Across \KSSourceFootprintWorkloads{} workloads with hand-written C/libbpf baselines, \tool{} sources total \KSSourceFootprintUniqueKSSloc{} lines versus \KSSourceFootprintUniqueCSloc{}. Table~\ref{tab:features} summarizes feature coverage.
...
Hand-porting \KSExternalPortWorkloads{} \texttt{xdp-tutorial}~\cite{xdp-tutorial} XDP programs to \tool{} yields \KSExternalPortKSSloc{} lines versus \KSExternalPortCSloc{} in the original C/eBPF. This confirms comparable line counts for external code.
% 手动将 \KSExternalPortWorkloads{} 个 \texttt{xdp-tutorial} XDP 程序移植到 \tool{} 产生 \KSExternalPortKSSloc{} 行，而原始 C/eBPF 为 \KSExternalPortCSloc{} 行。这确认外部代码的行数相当。
...
A direct libbpf runner bypassing skeletons loads both the generated and a matched hand-written tcp-congestion struct\_ops object successfully. This confirms toolchain version skew rather than an inherent limitation.
% 绕过骨架的直接 libbpf 运行器成功加载生成的和匹配的手写 tcp-congestion struct\_ops 对象。这确认是工具链版本偏差而非固有限制。
...
The change-amplification study uses small checked-in micro-edits instead of real revision histories. It therefore measures maintenance-surface coupling, not true developer time or debugging effort.
...
Finally, the struct\_ops build failures reflect bpftool/libbpf version skew, not a deep limitation. They do show that generated builds depend on toolchain alignment.
% 最后，struct\_ops 构建失败反映 bpftool/libbpf 版本偏差，而非深层限制。但它们表明生成的构建依赖于工具链对齐。
